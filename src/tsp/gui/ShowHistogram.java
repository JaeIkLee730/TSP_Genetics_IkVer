...
public class ShowHistogram{
	...
	public void show( ArrayList<Generation> genList ){

		showMinPathWeight( genList ) ;
	//	showAveragePathWeight() ;
	//	showSumOfPathWeight()

	}	

	private void showMinPathWeight( ArrayList<Generation> genList ){

//        ApplicationFrame frame = new ApplicationFrame("User Stat.") ;

 //       double [] minPW = new double[ genList.size() ] ;

	    for ( int i=0; i<genList.size(); i++ ) {
			for(int j=0; j<(genList.get(i).getMinPW())/1000; j++)
				System.out.print("*");
			System.out.println();
 //           minPW[i] = genList.get(i).getMinPW() ;
        }
	}
/*
        JFreeChart chart = getChart(genList);
        ...
        dataset.addSeries("Histogram", minPW, 20) ;
...
*/
	/*
	    public JFreeChart getChart( ArrayList<Generation> genList ) {
        ...
        // 데이터 입력 ( 값, 범례, 카테고리 )
        // 그래프 1       
		for(int i=0; i<genList.size(); ){
			String a = String.valueOf(i) ;
        	dataset1.addValue( (double)(genList.get(i).getMinPW()), "s1", a);
		}
